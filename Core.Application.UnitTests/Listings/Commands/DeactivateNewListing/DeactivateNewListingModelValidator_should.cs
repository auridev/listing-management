using Core.Application.Listings.Commands.DeactivateNewListing;
using FluentValidation.TestHelper;
using System;
using Xunit;

namespace Core.Application.UnitTests.Listings.Commands.DeactivateNewListing
{
    public class DeactivateNewListingModelValidator_should
    {
        private readonly DeactivateNewListingModelValidator _sut;
        public DeactivateNewListingModelValidator_should()
        {
            _sut = new DeactivateNewListingModelValidator();
        }

        [Fact]
        public void have_validation_error_if_ListingId_is_not_valid()
        {
            var model = new DeactivateNewListingModel()
            {
                ListingId = Guid.Empty
            };

            var result = _sut.TestValidate(model);

            result.ShouldHaveValidationErrorFor(model => model.ListingId);
        }

        [Theory]
        [InlineData(null)]
        [InlineData("")]
        [InlineData(
            @"")]
        public void have_validation_error_if_Reason_is_not_valid(string invalidReason)
        {

            var model = new DeactivateNewListingModel()
            {
                Reason = invalidReason
            };

            var result = _sut.TestValidate(model);

            result.ShouldHaveValidationErrorFor(model => model.Reason);
        }

        [Fact]
        public void not_have_any_validation_errors_if_all_properties_are_valid()
        {
            var model = new DeactivateNewListingModel()
            {
                ListingId = Guid.NewGuid(),
                Reason = "some valid reason"
            };

            var result = _sut.TestValidate(model);

            result.ShouldNotHaveAnyValidationErrors();
        }
    }
}
